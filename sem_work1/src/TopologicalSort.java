...
public class TopologicalSort{
    ...
    // Генерация датасета случайных DAG'ов разных размеров
    static void generateDatasets(String dir, int count, int nMin, int nMax) throws IOException {
        ...
        for (int i = 0; i < count; i++) {
            // Размер плавно растёт от nMin до nMax — чтобы график "время от N" имел смысл
            int n = nMin + (int) ((long) (nMax - nMin) * i / Math.max(1, count - 1));

            // Сначала строим DAG в "каноническом" порядке 0..n-1:
            ...
            // Настоящая случайная перестановка (Fisher–Yates через Collections.shuffle)
    ...
    public static void main(String[] args) throws Exception {
        ...
        try (PrintWriter out = new PrintWriter(resultFile)) {
            ...
            // Прогрев JIT: гоняем оба метода на нескольких первых файлах вхолостую
    ...
    // Чтение графа в обе структуры одновременно. try-with-resources — ресурс не утечёт.
